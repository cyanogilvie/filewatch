...
filewatch — Cross-platform filesystem event monitoring for Tcl

# SYNOPSIS

**package require filewatch**

**filewatch::watchdir create** *objectName* *dir*  
**filewatch::watchdir new** *dir*  
*objectName* **destroy**

Linux also provides the low-level inotify interface:
...
The **filewatch** package monitors files and directories for changes. It
provides a cross-platform **filewatch::watchdir** class that works on
Linux, macOS, and Windows, backed by each platform’s native filesystem
notification API:

| Platform | Backend                 |
|----------|-------------------------|
| Linux    | inotify(7)              |
| macOS    | FSEvents (CoreServices) |
| Windows  | ReadDirectoryChangesW   |

On Linux, the package is also available as **package require inotify**
for backwards compatibility, and the low-level **inotify::queue** class
is available for direct inotify access.

# filewatch::watchdir CLASS

The **filewatch::watchdir** class recursively watches a directory tree
for file creation and deletion. It is designed to be subclassed —
override the **new_file** and **remove_file** methods to respond to
events.
...
Called when a file is created or written. *normpath* is the full path
after **normalize_path** processing.

*objectName* **remove_file** *normpath*  
Called when a file is deleted or moved away. *normpath* is the full path
after **normalize_path** processing.
...
On Linux, the low-level **inotify::queue** class is available for direct
access to the inotify API. This is useful when you need fine-grained
control over which events to watch, or when monitoring individual files
rather than directory trees.
...
| **mask**   | A list of the flags that identify what the event is, from the tables below.                                                      |
...
a list containing flags from the event and add_watch flag tables below.
...
Masks select which event types to receive. The **Scope** column
...
Convenience flags: **IN_ALL_EVENTS** (all the above), **IN_MOVE**
(**IN_MOVED_FROM** \| **IN_MOVED_TO**), **IN_CLOSE** (**IN_CLOSE_WRITE**
\| **IN_CLOSE_NOWRITE**).

Additional flags valid only in the *mask* parameter to **add_watch**:
...
Additional flags that may appear in event masks:
...
| **IN_Q_OVERFLOW** | Event queue overflowed. See `/proc/sys/fs/inotify/max_queued_events`.                                                             |
| **IN_UNMOUNT**    | The filesystem containing the watched path was unmounted.                                                                         |

## Kernel limits

**/proc/sys/fs/inotify/max_queued_events**  
Events queued before **IN_Q_OVERFLOW** is generated.

**/proc/sys/fs/inotify/max_user_instances**  
Maximum queues per user.

**/proc/sys/fs/inotify/max_user_watches**  
Maximum watches per queue.

# PLATFORM SUPPORT

| Platform    | Status   | Backend               | Notes                                     |
|-------------|----------|-----------------------|-------------------------------------------|
| **Linux**   | Tested   | inotify(7)            | Full inotify::queue + filewatch::watchdir |
| **macOS**   | Untested | FSEvents              | filewatch::watchdir only                  |
| **Windows** | Untested | ReadDirectoryChangesW | filewatch::watchdir only                  |

The macOS and Windows backends have been developed and tested on Linux
using shim libraries that emulate each platform’s native API using
inotify. While this exercises the driver code paths end-to-end, it
cannot replicate the exact semantics of the real platform APIs.

**Testers with access to macOS or Windows are needed** to validate the
FSEvents and ReadDirectoryChangesW backends against the real platform
APIs. Please report any issues at the project’s issue tracker.

On all platforms, **filewatch::watchdir** provides the same interface.
On macOS and Windows, recursive watching is handled natively by the
kernel; on Linux, the watchdir manages per-directory inotify watches
internally.

# BUILDING

This package requires Tcl 8.6 or 9.0. The build system is `meson`, which
will find Tcl using its pkg-config file. Use the `PKG_CONFIG_PATH`
environment variable to point meson to it if Tcl is installed in a
nonstandard location.

## From a Release Tarball

Download and extract [the
release](https://github.com/cyanogilvie/filewatch/releases), then:

``` sh
tar xf filewatch-v2.3.tar.gz
cd filewatch-v2.3
...
Fetch [the code](https://github.com/cyanogilvie/filewatch) and
submodules recursively, then build:

``` sh
git clone --recurse-submodules https://github.com/cyanogilvie/filewatch
cd filewatch
meson setup builddir --buildtype=release
meson compile -C builddir
meson install -C builddir
...
To test a non-native backend on Linux using the inotify shim:

``` sh
meson setup builddir_xnu -Ddriver=xnu
meson test -C builddir_xnu

meson setup builddir_nt -Ddriver=nt
meson test -C builddir_nt
```
...
inotify(7), fsevents(3), ReadDirectoryChangesW

# KEYWORDS

file, notification, event, inotify, fsevents, filewatch, watchdir, tcl
