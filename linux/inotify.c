...
#define ASSOC_KEY	PACKAGE_NAME "::inotify"
...
static struct cmd {
	...
	{ NS_DRIVER_STR "::create_queue",	cmd_create_queue	},
	{ NS_DRIVER_STR "::add_watch",		cmd_add_watch		},
	{ NS_DRIVER_STR "::rm_watch",		cmd_rm_watch		},
	{ NS_DRIVER_STR "::decode_events",	cmd_decode_events	},
	{0}
};
...
int Inotify_Init(Tcl_Interp* interp) //<<<
{
	...
	Tcl_CreateEnsemble(interp, NS_DRIVER_STR, ns, 0);

	struct interp_cx* l = (struct interp_cx*)ckalloc(sizeof *l);
	*l = (struct interp_cx){0};
	Tcl_SetAssocData(interp, ASSOC_KEY, free_interp_cx, l);
	...
	TEST_OK_LABEL(finally, code, pkgdir_path(interp, "queue.tcl", &script_fn));
	...
	TEST_OK_LABEL(finally, code, Tcl_PkgProvide(interp, "inotify", PACKAGE_VERSION));

finally:
	if (code != TCL_OK) {
		if (ns) { Tcl_DeleteNamespace(ns); ns = NULL; }
		Tcl_SetAssocData(interp, ASSOC_KEY, NULL, NULL);
	}
